...
describe('WebChess public website', () => {
  it('explains the complete method and links into the playable game', () => {
    ...
    expect(screen.getByText(/configured model first proposes 64 bounded perspectives/i)).toBeInTheDocument()
    ...
    expect(screen.getByText(/final model receives the original problem, outcome and game totals/i)).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: /two model passes\. two distinct jobs/i })).toBeInTheDocument()
    expect(screen.queryByRole('heading', { name: /two calls/i })).not.toBeInTheDocument()
