...
import { access, cp, lstat, mkdir, mkdtemp, open, readFile, readdir, rm, symlink, } from 'node:fs/promises';
...
export const WEBCHESS_LOCAL_DATA_NOTICE = 'WebChess software 2.2.0-rc.1 stores game history and the webchess-2.0 lifecycle schema in the dedicated local PostgreSQL database. Inference and official Codex Hosted Search use your selected OpenAI account/OAuth profile and contact OpenAI; provider key/token fallbacks are rejected.';
...
async function runtimePayloadIdentity(root) {
    const files = [];
...
export async function resolveWebChessBuildIdentity(sourceRoot) {
    ...
    try {
        ...
        if (parsed.format !== 'webchess-build-identity/1' ||
...
function dedicatedDatabaseUrl(environment) {
    ...
    if (rawValue !== value) {
        throw new Error('WEBCHESS_OPENCLAW_DATABASE_URL must not contain surrounding whitespace.');
    }
    let parsed;
    try {
    ...
    if (!['postgres:', 'postgresql:'].includes(parsed.protocol) ||
        ...
        throw new Error('WEBCHESS_OPENCLAW_DATABASE_URL must use PostgreSQL on a numeric loopback host.');
    }
    if (value.includes('?') || value.includes('#')) {
        throw new Error('WEBCHESS_OPENCLAW_DATABASE_URL must not contain query parameters or a fragment.');
    }
    ...
    if (!componentsAreValid || databaseComponent.includes('/') ||
        ...
        throw new Error('WEBCHESS_OPENCLAW_DATABASE_URL must include one username, password, explicit port, and database name.');
    }
    return parsed.href;
}
...
export function buildNextLaunchSpec(root, options, environment = process.env, nextBinary = resolveNextBinary(), identity, bridge, buildIdentity) {
    if (!bridge) {
    ...
    Object.assign(localEnvironment, {
        ...
        // These are protocol policy, not user tuning: the provider-only portion of
        // one model turn is bounded to 150 seconds inside its authenticated
        // envelope, while one consented Hosted Search receives five minutes.
        ...
        ...(buildIdentity
            ? {
                WEBCHESS_RUNTIME_ARTIFACT_SHA256: buildIdentity.runtimeArtifactSha256,
            }
            : {}),
    });
    return {
...
export async function launchWebChess(options, dependencies = defaultDependencies, api) {
    ...
    try {
        ...
        if (stagedBuildIdentity.runtimeArtifactSha256 !==
            sourceBuildIdentity.runtimeArtifactSha256) {
            throw new Error('The staged WebChess runtime does not match the verified source payload.');
        }
        const buildIdentity = {
        ...
        const spec = buildNextLaunchSpec(runtimeRoot, options, dependencies.environment, nextBinary, identity, bridge, buildIdentity);
