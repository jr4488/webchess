...
import {
  attestPinnedOpenClawPreparedAuthAccountInspector,
...
export interface OpenClawWebSearchProvider {
  applySelectionConfig(config: OpenClawRuntimeConfig): OpenClawRuntimeConfig
  ...
  hint: string
  id: string
  inactiveSecretPaths: readonly string[]
  label: string
  onboardingScopes?: readonly string[]
  placeholder: string
  pluginId: string
  requiresCredential?: boolean
  runSetup(context: unknown): Promise<unknown>
  setCredentialValue(
...
export type CodexPluginRecordResolver = (
  environment: NodeJS.ProcessEnv,
  stateDir: string,
  workspaceDir: string,
...
export interface WebChessBridgeOptions {
  ...
  preparedAuthAccountInspector?: PreparedAuthAccountInspector
...
function hasStableOAuthIdentity(
...
async function hasBoundPreparedOpenAiAccount(
...
const CODEX_PROVIDER_FUNCTION_KEYS = new Set<string>([
  'applySelectionConfig',
  'createTool',
  'getCredentialValue',
  'runSetup',
...
function isExactCodexProviderContract(
  provider: OpenClawWebSearchProvider,
): boolean {
  ...
  if (Object.getPrototypeOf(provider) !== Object.prototype ||
    ...
    !isDeepStrictEqual(
      ...
      [...CODEX_PROVIDER_CONTRACT_KEYS].sort(),
    ) || !CODEX_PROVIDER_CONTRACT_KEYS.every((key) => {
  ...
  return provider.id === 'codex' &&
    ...
    [...CODEX_PROVIDER_FUNCTION_KEYS].every((key) =>
      typeof (provider as unknown as Record<string, unknown>)[key] ===
        'function')
}

function hasSameCodexProviderContract(
  initial: OpenClawWebSearchProvider,
  current: OpenClawWebSearchProvider,
): boolean {
  if (!isExactCodexProviderContract(initial) ||
    !isExactCodexProviderContract(current)) return false
  const left = initial as unknown as Record<string, unknown>
  const right = current as unknown as Record<string, unknown>
  return CODEX_PROVIDER_CONTRACT_KEYS.every((key) =>
    CODEX_PROVIDER_FUNCTION_KEYS.has(key)
      ? left[key] === right[key]
...
async function resolveBoundCodexSearchProvider(
  ...
  recordResolver: CodexPluginRecordResolver,
  environment: NodeJS.ProcessEnv,
  stateDir: string,
  workspaceDir: string,
  emptyEnvironmentNames: readonly string[],
  ...
  try {
    ...
    const record = await recordResolver(
      environment,
      stateDir,
      workspaceDir,
...
function hasCompatiblePluginConfig(config: OpenClawRuntimeConfig): boolean {
  ...
  if (!isRecord(plugins) || !Array.isArray(plugins.allow) ||
    plugins.allow.length !== 3) return false
...
async function prepareOpenAiAccountModel(
  ...
  if (!expectedOAuthIdentity || !await hasBoundPreparedOpenAiAccount(
    prepared,
    expectedOAuthIdentity,
...
export async function startWebChessBridge(
  ...
  const assertNoDrainingModelExecution = () => {
    if (drainingModelExecutions.size === 0) return
    throw new BridgeRequestError(
      503,
      'MODEL_PROVIDER_DRAINING',
      'A previous model turn is still draining after cancellation.',
    )
  }
  ...
  const server = createServer((request, response) => {
    const run = (async () => {
      ...
      const resetRequestDeadline = (deadlineAt: number) => {
        clearTimeout(timeout)
      ...
      try {
        if (request.url === '/v1/model/run') {
          ...
          const modelRequestDeadline =
            requestStartedAt + MODEL_REQUEST_ENVELOPE_TIMEOUT_MS
          ...
          let providerDeadlineAt: number | null = null
          const markProviderTimedOut = () => {
            if (providerTimedOut) return
            providerTimedOut = true
            providerController.abort(
              new Error('WebChess bridge provider timeout'),
            )
          }
          ...
          const retainDrainingModelExecution = (): boolean => {
            const execution = modelProviderExecution.current
            if (!execution) return false
            if (modelTurn) modelTurn.state = 'draining'
          ...
          try {
            const completion = await raceProviderExecution(
              ...
              providerSignal,
              () => providerDeadlineExpired() || expireElapsedDeadline(),
            )
