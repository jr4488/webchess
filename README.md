...
8. **Answer** receives the exact Gate-approved prompt plus a versioned,
   digest-bound projection of the full directional record: the eight surviving
   signals, their scoring/support provenance, explanation, and usable Portia
   amendments. The complete record remains durable for replay/export.
9. **Charlotte** turns that stored Answer into one standalone corrected final
   answer from the same directional projection, visibly applies its material
   qualifications, and proposes exactly three reversible actions. The earlier
   Answer remains visible only as provenance; no later correction stage exists.
...
6. inspect the trajectory-direction record and its digest, how **What survived
   ...
   Retry ancestry if any, the approved Answer retained as provenance, and the
   standalone corrected final answer after Charlotte review;
...
For each Portia candidate whose response fails the strict structural or
semantic contract, WebChess discards it and may issue exactly one corrective
candidate turn before failing that run-level attempt. The correction reuses the
unchanged trusted input, has a distinct idempotency identity, renews the same
durable lease, and never copies the rejected provider text. A run with `S`
survivors can therefore add up to `S` corrective generations. If a correction
also fails, the incomplete candidate remains unpersisted and the existing
three-attempt Portia budget applies; across that budget, candidate corrections
can add at most `3S` generations, in addition to any repeated nominal candidate
and summary calls. Provider, transport, and cancellation failures do not earn a
semantic correction.
...
| Current method-version tuple | lifecycle `webchess-lifecycle-v2.5`; Division `webchess-division-v5`; Portia `webchess-portia-v6`; Portia review `webchess-portia-review-v3`; Gate `webchess-gate-v5`; Answer `webchess-answer-v5`; Charlotte `webchess-charlotte-v7` | One canonical tuple in [`src/lib/lifecycle/method-versions.mjs`](src/lib/lifecycle/method-versions.mjs), copied into and strictly verified by the release identity |
